...
class LoadsTests(unittest.TestCase):
    def test_empty(self):
        self.assertEqual(cpghstore.loads(''), {})
    def test_simple(self):
        self.assertEqual(cpghstore.loads('"key" => "value"'), {"key": "value"})

        self.assertEqual(
            cpghstore.loads('"key" => "value", "key2" => "value2"'), 
            {"key": "value", "key2": "value2"});
    def test_escaped_double_quote(self):
        self.assertEqual(
            cpghstore.loads('"k\\"ey" => "va\\"lue"'), 
            {"k\\\"ey": "va\\\"lue"});
    def test_null(self):
        self.assertEqual(cpghstore.loads('"key" => null'), {"key": None})
        self.assertEqual(cpghstore.loads('"key" => NULL'), {"key": None})    
        self.assertEqual(cpghstore.loads(
    ...
    def test_utf8(self):
        self.maxDiff = None
        #self.assertEqual(cpghstore.loads('"åäö" => "åäö"'), {"åäö": "åäö"})
        s = '"name"=>"Noorwe\xc3\xab", "name2"=>"öäå"'
        self.assertEqual(cpghstore.loads(s), 
                         {"name": "Noorwe\xc3\xab",
                          "name2": "öäå"})
        ...
        r = cpghstore.loads(names)
        self.assertEqual(
            cpghstore.loads(names),
